...
'''Period 1 & 2: 15 market open days (not including the most previous market open day) to calculate the orginal upward/downward movement averages'''
...
'''2nd period lists to use to calculate the Up/Down movement average (initial_up_movement*13 + previous_upchangetwo)/14)'''
...
'''Reading just the Close columns'''
...
'''Turning Close columns into float values to put in list/array'''
float_closes = list(map(float, closes))
float_closestwo = list(map(float, closestwo))
#print(float_closes)

'''Finding the daily changes between each day (14 changes)'''
...
'''Downward movements must be positive in the RSI calculation; turning the negative movements into positive using absolute value'''
...
'''Finding the initial upward movement average for the second most previous day'''
initial_up_movement = sum(upward_movements)/len(upward_movements)
print(len(upward_movements))

'''Calculating the up movement avg for the previous day'''
